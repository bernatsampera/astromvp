...
export default function ListDisplay({ cityId, gyms }: { cityId: number | null; gyms: GymType[] }) {
  if (!cityId) {
    return <div> Select a city </div>;
  }

  if (gyms.length === 0) {
    return <div> There are no Gyms for the selected City. Option to add a gym coming soon! </div>;
  }
